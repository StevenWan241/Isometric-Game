# Tiny World Adventure – Isometric Action Game

## Overview
Tiny World Adventure is an isometric 2.5D action-puzzle game built in Unity. It features a grid-based movement system, collectible items, enemy AI, boss-level projectile patterns, and smooth camera transitions using Cinemachine. This project demonstrates gameplay mechanics, environmental interaction, and basic level design principles.

---

## How to Open the Project

### Option 1 – Open in Unity
1. Download or clone the repository
2. Open **Unity Hub**
3. Click **Add Project**
4. Select the downloaded folder
5. Open using **Unity version: 2023.1.11f1**

### Option 2 – Unity Package
1. Create a **new Unity project**
2. Go to **Assets → Import Package → Custom Package**
3. Select `Isometric Game.unitypackage`
4. Import all files

---

## How to Play

**Controls**
- **WASD / Arrow Keys** – Move player  
- **Space** – Interact with cannon / Push objects
- **Left or Right Arrow Keys** – Switch aiming position 
- **Escape** – Exit aim mode 

**Objectives**
- Collect items (coins, keys, health potions)  
- Solve environmental puzzles  
- Defeat the boss using cannon mechanics  
- Navigate through the gates to complete each level  

---

## Project Features
- Grid-based isometric movement system  
- Enemy patrol AI with chase behavior  
- Boss with patterned projectile attacks  
- Interactive block-pushing mechanics  
- Cannon aiming system with camera transitions  
- Layered collision system for accurate isometric physics  
- Simple UI for health and interactive elements  

---

## Notes
This project demonstrates my ability to design interactive gameplay, iterate on mechanics based on testing, and create clear and playable systems in Unity. Future improvements include additional levels, more enemy patterns, and expanded puzzle mechanics.

